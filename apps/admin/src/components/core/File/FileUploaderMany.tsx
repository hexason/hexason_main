...
export const FileUploaderMany = ({ imgs, onChange }: { imgs?: ImageUploadManyType[], onChange?: (prop: ImageUploadManyType[]) => any; }) => {
  ...
  const handleUploader = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < e.target.files.length; i++) {
      preImages.push({ name: e.target.files[i].name, url: "", isUploaded: false, file: e.target.files[i] });
    }
    const files = new Queue<ImageUploadManyType, ImageUploadManyType[]>(preImages.filter(m => !m.isUploaded), logger);
  ...
  return (
    <Wrap>
      ...
      <Button isLoading={loading} colorScheme="blackAlpha" position={"relative"}>
        +
        <Input
          type="file"
          height="100%"
